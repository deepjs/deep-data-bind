...
define(["require","deepjs/deep", "deepjs/lib/view"], function(require, deep)
{
	deep.errors.Bind = function(msg, report, fileName, lineNum) {
		...
		if (!msg)
			msg = "RouterError : ";
		return this.Error(3000, msg, report, fileName, lineNum);
	};
	...
	deep.ui.View.htmlEnhancers["dp-bind"] = function (node) {
		var type = $(node).attr("dp-bind");
		switch(type)
		{
			case "property" :
				deep.ui.bind(node, $(this).attr("dp-path"), {
					directPatch: $(this).attr("dp-auto-patch")?true:false,
					callback: null
				});
			break;
			case "input" :
				deep.ui.bindInput(node, $(this).attr("dp-path"), {
					directPatch: $(this).attr("dp-auto-patch")?true:false,
					callback: null
				});
			break;
			case "object" :
				var uri = $(this).attr("dp-path");
				...
				deep.getAll([uri,schemapath]).done(function (res) {
					deep.ui.toJSONBind(res[0], node, res[1] || {});
				});
			break;
			default :
				return deep.errors.Bind("no bind found with (dp-bind) : "+type);
		}
	};
